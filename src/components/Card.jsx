import React from "react";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import clsx from "clsx";
import Button from "./Button.jsx";
import button from "./Button.jsx";

const Card = ({
  icon,
  image,
  title,
  highlight,
  desc,
  name,
  additionalClassName,
  button,
}) => {
  const Inner = () => (
    <span>
      {icon && (
        <span
          className={"flex flex-col justify-between items-start gap-8 h-full"}
        >
          <h4 className="highlight mt-32">{highlight}</h4>
          <h4 className="text-4xl font-medium">{title}</h4>
          <FontAwesomeIcon
            icon={icon}
            className="absolute text-4xl border-2 rounded-full p-2 border-p16 top-10 left-10 bg-l2"
          />
          <p className="text-2xl">{desc}</p>
        </span>
      )}
      {image && (
        <span className="flex gap-3 justify-center items-center">
          <div>
            <img src={image} alt={name} className="h-32 w-32 bg-l2" />
          </div>
          <div>
            <p className="text-sm leading-snug">{desc}</p>
            <p>{name}</p>
          </div>
        </span>
      )}
      {button && (
        <Button additionalClassName="bg-s1 absolute bottom-4 text-p1">
          {button}
        </Button>
      )}
    </span>
  );
  return (
    <div
      className={clsx(
        "transition-all duration-500 ease-in-out border relative rounded-lg flex flex-col items-center justify-start",
        additionalClassName,
      )}
    >
      <div className={"h-16 w-0.5 bg-p16 absolute left-16"} />
      <Inner />
    </div>
  );
};
export default Card;
